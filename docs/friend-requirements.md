...
ブロックの遮断は「`Friendship` 行が消えることで、フレンド前提の機能がすべて止まる」という形で成立している（DM・招待・presence）。**ito のルーム参加だけはこの形に乗らない。** ルームはフレンド関係を前提にせず、コードさえ知っていれば誰でも入れるため（[room-invite-requirements.md](docs/room-invite-requirements.md) セクション0）、フレンドを消しても参加は止まらない。招待の送信が `assertFriends` で止まるだけで、コードが別経路で伝われば同席できてしまう。

そのため `joinRoom` に唯一のDB参照として判定を1つ足している（[room.service.ts](backend/src/ito/service/room.service.ts) の `isBlockedByHost`）。

- **判定相手はホストのみ。** その部屋で誰と同席するかを決める権利を持つのはホストだから。ホスト以外の参加者との関係まで見ると、先に入った人は後から来る人を弾ける一方、既に居る人は追い出せないという非対称が生まれる
- **向きは問わない**（`FriendsService.areBlockedEitherWay`）。ホストが参加者をブロックしている場合も、参加者がホストをブロックしている場合も弾く。「ブロックした相手とは同席しない」という一貫した意味になる
- **文言は「このルームには参加できません」。** 満員・開始済みと区別が付かない代わりに、ブロックの存在を相手に知らせない（上記「ブロックの存在自体を隠す」と揃える）
- **判定できなかった場合（DB障害など）は拒否に倒す。** 確かめられないまま通すと遮断が意味をなさないため
- **ゲーム開始後にブロックしても、その回の同席は解消されない。** 参加時点の1回だけ判定する。進行中の席を奪うと turnOrder / boardOrder の再計算が要り、ホストの「除外」操作と役割が重複する。次のルームから効く

判定を入れられるのは `/ito` が JWT 認証必須になり `playerId` をトークンから導出するようになった後だから、という前提もある。クライアントの申告値だった頃は、名乗る `playerId` を変えるだけで判定を回避できた。
